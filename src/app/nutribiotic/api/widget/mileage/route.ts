...
import { setRouteMileageDay } from "../../../lib/dal";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const today = (
      await setRouteMileageDay(day, {
        [kind]: { odo, driveFileId: uploaded.driveFileId, photoLink: uploaded.photoLink, capturedAt: new Date().toISOString() },
      })
    )[day] ?? {};

    // Only one side so far: nothing more to do until the other one lands.
    if (!today.start || !today.end) {
      return Response.json({ ok: true, filed: false, odo, photoLink: uploaded.photoLink });
    }
    ...
    if (today.start.odo && today.end.odo) {
      try {
        ...
        return Response.json({ ok: true, filed: true, sheetLink: filed.sheetLink, miles: filed.miles, odo });
      } catch (e) {
        ...
        return Response.json({
          ...
          filed: false,
          odo,
          error: `Both photos saved, but filing the trip failed: ${message}`,
        });
      }
    }

    // Both photos are in, but at least one odometer reading wasn't legible.
    // Never guess it: leave both links in place and say so.
    await setRouteMileageDay(day, { fileError: "One odometer reading wasn't legible. Enter it by hand on /nutribiotic/expenses." });
    return Response.json({
      ok: true,
      filed: false,
      odo,
      error: "Both photos saved. One odometer reading wasn't legible -- enter it by hand on /nutribiotic/expenses.",
    });
  } catch (e) {
